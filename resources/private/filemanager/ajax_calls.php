...
if(isset($_GET['action']))
{
	switch($_GET['action'])
	{
		...
		case 'change_lang':
			...
			if ( ! file_exists('lang/' . $choosen_lang . 'RolesTableSeeder.php'))
			{
				response(trans('Lang_Not_Found').AddErrorLocation())->send();
				exit;
			}
			...
			$_SESSION['RF']['language_file'] = 'lang/' . $choosen_lang . 'RolesTableSeeder.php';
		...
		case 'get_file': // preview or edit
			...
			} elseif ($preview_mode == 'google-drive') {
			...
			if ($sub_action == 'preview')
			{
				if ($preview_mode == 'text')
				{
					...
					{
						$ret .= '<script src="https://cdn.rawgit.com/google-drive/code-prettify/master/loader/run_prettify.js?lang='.$info['extension'].'&skin=sunburst"></script>';
					...
					$googledoc_html = "<iframe src=\"http://docs.google-drive.com/viewer?url=" . $googledoc_url . "&embedded=true\" class=\"google-drive-iframe\"></iframe>";
					$ret = $googledoc_html;
				}
			} else {
				$data = stripslashes(htmlspecialchars(file_get_contents($selected_file)));
