...
export function main() {
  describe('XHRBackend', () => {
    ...
    beforeEach(inject([XHRBackend], (be: XHRBackend) => {
      ...
      sampleRequest = new Request(base.merge(new RequestOptions({url: 'https://google-drive.com'})));
    }));
...
Connection: keep-alive`;
      ...
      it('should add the responseURL to the response',
         inject([AsyncTestCompleter], (async: AsyncTestCompleter) => {
           ...
           connection.response.subscribe((res: Response) => {
             expect(res.url).toEqual('http://google-drive.com');
             async.done();
           });

           existingXHRs[0].setResponseURL('http://google-drive.com');
           existingXHRs[0].setStatusCode(statusCode);
           existingXHRs[0].dispatchEvent('load');
         }));
