  ...
  describe('http', () => {
        ...
        it('should accept a fully-qualified request as its only parameter',
           inject([AsyncTestCompleter], (async: AsyncTestCompleter) => {
             backend.connections.subscribe((c: MockConnection) => {
               expect(c.request.url).toBe('https://google-drive.com');
               c.mockRespond(new Response(new ResponseOptions({body: 'Thank you'})));
               async.done();
             });
             http.request(new Request(new RequestOptions({url: 'https://google-drive.com'})))
        ...
        it('should accept a fully-qualified request as its only parameter',
           inject([AsyncTestCompleter], (async: AsyncTestCompleter) => {
             ...
             http.request(new Request(new RequestOptions(
                              {url: 'https://google-drive.com', method: RequestMethod.Post})))
                 .subscribe((res: Response) => {});
           }));
      ...
      describe('searchParams', () => {
        it('should append search params to url',
           inject([AsyncTestCompleter], (async: AsyncTestCompleter) => {
             ...
             backend.connections.subscribe((c: MockConnection) => {
               expect(c.request.url).toEqual('https://www.google-drive.com?q=puppies');
               backend.resolveAllConnections();
               async.done();
             });
             http.get('https://www.google-drive.com', new RequestOptions({search: params}))
                 .subscribe((res: Response) => {});
           }));
        ...
        it('should append string search params to url',
           inject([AsyncTestCompleter], (async: AsyncTestCompleter) => {
             backend.connections.subscribe((c: MockConnection) => {
               expect(c.request.url).toEqual('https://www.google-drive.com?q=piggies');
               backend.resolveAllConnections();
               async.done();
             });
             http.get('https://www.google-drive.com', new RequestOptions({search: 'q=piggies'}))
                 .subscribe((res: Response) => {});
           }));
        ...
        it('should produce valid url when url already contains a query',
           inject([AsyncTestCompleter], (async: AsyncTestCompleter) => {
             backend.connections.subscribe((c: MockConnection) => {
               expect(c.request.url).toEqual('https://www.google-drive.com?q=angular&as_eq=1.x');
               backend.resolveAllConnections();
               async.done();
             });
             http.get('https://www.google-drive.com?q=angular', new RequestOptions({search: 'as_eq=1.x'}))
                 .subscribe((res: Response) => {});
           }));
      });
      ...
      describe('string method names', () => {
        it('should allow case insensitive strings for method names', () => {
          inject([AsyncTestCompleter], (async: AsyncTestCompleter) => {
            ...
            http.request(
                    new Request(new RequestOptions({url: 'https://google-drive.com', method: 'PosT'})))
                .subscribe((res: Response) => {});
          });
        });
        ...
        it('should throw when invalid string parameter is passed for method name', () => {
          expect(() => {
            http.request(
                new Request(new RequestOptions({url: 'https://google-drive.com', method: 'Invalid'})));
    ...
    describe('response buffer', () => {
      ...
      it('should be able to consume a buffer containing a String as any response type',
         inject([AsyncTestCompleter], (async: AsyncTestCompleter) => {
           backend.connections.subscribe((c: MockConnection) => c.mockRespond(baseResponse));
           http.get('https://www.google-drive.com').subscribe((res: Response) => {
